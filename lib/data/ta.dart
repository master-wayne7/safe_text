/// Bundled word list for language code `ta`.
const List<String> kTaWords = <String>[
  'aayava ootha alla sunni',
  'akkula thukki kami di',
  'alugina papali',
  'amarjith',
  'anaathai kaluthai',
  'arippu edutha kuthi mavale',
  'arivu ketta koothi',
  'auvusaari koodi',
  'avuthu kami',
  'baadu',
  'blousei thukkudy',
  'blue satta maran',
  'chunni',
  'enna oombuda elvedutha koodi',
  'ennoda poola oombuda',
  'erumbu',
  'gajak kol',
  'gotha',
  'gotha chunni umbu',
  'hakka kudi',
  'kaai',
  'kaai adithal',
  'kaai amuki',
  'kadiki',
  'kala viridi',
  'kalai viridi',
  'kalati podri',
  'kaluthae',
  'kamnaati',
  'kandaara oli',
  'kandaloli',
  'kanji',
  'karum pundai',
  'karung kuthi mavale',
  'kazhudhaiya otha naaye',
  'keeshan appa',
  'kena',
  'kena punda',
  'ki adi',
  'koodhi',
  'koodhi payale',
  'koothi mayir',
  'koothi nuckie',
  'kudiya baadu',
  'kundi',
  'kundi mudi',
  'kunja thingira',
  'kunji payalae',
  'kunju',
  'kusu koodhi',
  'kuthi kolutha thevdia',
  'kuthi mudiya thadavi kami di',
  'kuthia virikira thevdia',
  'loosu koodhi',
  'mairu pudungi',
  'makki punda',
  'makku koodhi',
  'mala mattu punda',
  'malayalathan atha oli',
  'mayire',
  'mayiru poodunghi',
  'mola sappi',
  'molai',
  'molaikku naduvule uttu okka',
  'molaikku naduvule utu olu da',
  'mollamaari',
  'monna naaye',
  'mukree chodho',
  'munda kalappai',
  'munni thalai',
  'muttaa koodhi',
  'naara koodhi',
  'naara kudhi pethadha',
  'naii',
  'nalla thukki kami di nara kuthia',
  'narum pundai',
  'nayee soothile un kunji',
  'neen poranthathunale enakku eenam a pochu',
  'oobhu',
  'oolu',
  'oomala',
  'otha',
  'othalakka',
  'pochandi',
  'pochu',
  'poole',
  'punda',
  'pundanakki',
  'pundarani',
  'savugraaki',
  'sunni',
  'thaevudiya',
  'thaiyoli',
  'thevdiya',
  'ungathaalaokku',
  'உங்காத்தாலவ்க்கு',
  'ஊபு',
  'ஊமல',
  'ஓத',
  'ஓதாளக்க',
  'ஓலு',
  'கண்டாலொழி',
  'கம்னாட்டி',
  'குஞ்சு',
  'கூதி',
  'கேன',
  'கோத்த',
  'சவுக்கிராகி',
  'சுன்னி',
  'தேவடியா',
  'தேவுடியா',
  'தையொலி',
  'பாடு',
  'புண்டரணி',
  'புண்டை',
  'புண்டைனாக்கி',
  'பூலே',
  'போச்சண்டி',
  'போச்சு',
];
