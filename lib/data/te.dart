/// Bundled word list for language code `te`.
const List<String> kTeWords = <String>[
  'aathulu',
  'aathulu peeku',
  'aathulu peekutha',
  'aati mokka',
  'aatu leni mogga',
  'adivi pooku',
  'akka bokka lo naa sulla',
  'akka nee pooku dengutha',
  'amma pooku ki baanisa',
  'amma pooku lo arati pandu',
  'aritipandu mogga',
  'attha pooku dengina alldu',
  'banda pooku',
  'bochu',
  'bokka',
  'bosudi',
  'chanu mona',
  'chilipiga thittatam',
  'chitthu pooka',
  'cutlet etthi guddha lo pettuko',
  'daddu',
  'dengithe dayyalu pudatai',
  'dengu',
  'dhani puku lo erugu',
  'dimma',
  'doola lanja',
  'erri pooka',
  'erugu',
  'gaadida pichaloda',
  'gaja madda',
  'gajji modda',
  'golli cheeku ra kodaka',
  'golli chekutha lanja',
  'gudda',
  'gudda naaku',
  'guddha cheeku',
  'guddha naku',
  'guddha pagala dengutha',
  'gudha kindha kovvu pattindha',
  'hasta prayogam chai',
  'jinniappa',
  'kammani pooku',
  'kojja',
  'kojja munda',
  'konamodda',
  'konda verri pooku',
  'kukka -pooka',
  'kukka sulli',
  'lafoot lanja',
  'langa vippavae lanjaa',
  'lanja',
  'lanja munda',
  'lanja pooka',
  'lanjakodka',
  'lavada',
  'magatanam',
  'marmangam',
  'modda',
  'modda cheeku lanja',
  'modda kuduvu',
  'modda naaku',
  'modda notlo petti dengutha',
  'muchchika',
  'muddy lo velu petuko',
  'naa modda gudu',
  'ne akka nu denga',
  'ne akkan ma kukka dengha',
  'ne amma ni denga',
  'ne amma pukkulo naa modda',
  'ne jathini kukka denga',
  'ne notlo naa suli',
  'ne pookula na sulli',
  'nee aali pookulo rampam',
  'nee akkaani dengaa',
  'nee akkanu denga',
  'nee alini dengaa',
  'nee amma guddani denga',
  'nee amma kuttani denga',
  'nee amma roju na modda cheekuthundi',
  'nee ammani dengaa',
  'nee ammanu denga',
  'nee ammanu dnegu',
  'nee kooturu ni denga',
  'nee notlo naa sulli',
  'nee pallu super',
  'nee payal madhyalo na sulli',
  'nee pellaanni denga',
  'nee pooku chekutha',
  'nee pooku lo na gudda',
  'nee pukulo naa sulli',
  'ni akkani pandulu denga',
  'ni gudha la gudu pedtha',
  'ni pukla na madda',
  'nihar',
  'ninnu dengutha',
  'nuvvu nee yendipoyina sulli',
  'paaki munda',
  'pappa',
  'pedda salladhi',
  'pichi pukoda',
  'pichi pukudaana',
  'piyya thinnu',
  'poi voungu ra',
  'pooku',
  'pooku pagaldenguta',
  'poooku chinchu',
  'pucci na kodaka',
  'pukku naaku',
  'puku peka adistha',
  'puthhi',
  'sachinoda',
  'sallu dengutha',
  'sanalo bochho',
  'shaata',
  'sravan',
  'sulii',
  'sulli pattu',
  'teja',
  'teja trivikram',
  'thurku valagodtha',
  'vattakai',
  'vattalu',
  'vedhava',
  'verri puka',
  'vongo petti dengutha lanja',
  'x రేట్',
  'అంగ',
  'అంగస్తంభన',
  'అంబిసెక్సువల్',
  'అంబోసెక్సువల్',
  'అక్రమ సంబంధం',
  'అక్రోటోమోఫిలియా',
  'అచ్చు',
  'అత్యాచారం',
  'అశ్లీల',
  'అసాంఘిక కార్యకలాపాలు',
  'అహంకారం',
  'ఆంబిసెక్సువాలిటీ',
  'ఆత్మవిశ్వాసం',
  'ఆర్గాసిమ్',
  'ఆర్గీస్',
  'ఆర్గ్',
  'ఇబ్బంది పెట్టాడు',
  'ఉన్నప్పటికీ బలాత్కారం',
  'ఎజాక్యులేటెడ్',
  'ఎజాక్యులేట్స్',
  'ఎరాక్యులేటింగ్',
  'ఒంటి',
  'ఒక బిచ్ కుమారుడు ఆఫ్',
  'కం',
  'కంట్',
  'కాక్',
  'కాక్-సక్కర్',
  'కాక్స్',
  'కామం',
  'కార్పెట్ ముంచర్',
  'కుక్క-fucker',
  'కుడుచుట',
  'కుదుపు',
  'కున్నిలింగస్',
  'కూన్',
  'క్యునిల్లింగస్',
  'క్లిటోరిస్',
  'గర్భస్రావం',
  'గర్భస్రావవాది',
  'గర్భాశయం',
  'గాడిద',
  'గాడిద-fucker',
  'గాడ్డామ్',
  'గుదము',
  'గుబ్బ',
  'చనుమొన',
  'చింక్',
  'చెత్త',
  'చొచ్చుకుపోవడం',
  'ఛాతీ',
  'జననాంగం',
  'జూఫిలియా',
  'జూఫైల్',
  'జెర్క్-ఆఫ్',
  'టిట్',
  'టిట్స్',
  'డిక్',
  'డిల్డోస్',
  'డూష్',
  'డైక్',
  'తల్లి ఫెకర్',
  'తిట్టు',
  'తునక',
  'దూరు',
  'దేవతలా మంచిది',
  'నరకం',
  'నవుకరు',
  'నాజీ',
  'నాబ్స్',
  'నింఫోమానియా',
  'నింఫ్',
  'నిగ్గర్',
  'నిగ్గర్స్',
  'నిటారుగా',
  'నీతి',
  'పాన్సెక్సువల్',
  'పాయువు',
  'పిస్సర్',
  'పిస్సాఫ్',
  'పిస్సింగ్',
  'పిస్సీస్',
  'పీడన కాముకత',
  'పీడోఫైల్',
  'పురీషనాళం',
  'పురుషాంగం',
  'పుస్సీ',
  'పెడోఫిలియాక్',
  'పెడోఫీలియా',
  'పెదవి',
  'పోర్నో',
  'పోర్నోగ్రఫీ',
  'ప్యూబ్',
  'ప్రిక్స్',
  'ఫకర్స్',
  'ఫకింగ్',
  'ఫకింగ్స్',
  'ఫక్',
  'ఫక్స్',
  'ఫగాట్స్',
  'ఫడ్జ్ ప్యాకర్',
  'ఫాగాట్',
  'ఫాగ్గింగ్',
  'ఫెటిష్',
  'ఫెల్చింగ్',
  'బం',
  'బంతుల్లో',
  'బట్',
  'బాల్బ్యాగ్',
  'బాస్టర్డ్',
  'బిచ్',
  'బిట్చింగ్',
  'బుడతడు',
  'బూబ్',
  'బెల్లెండ్',
  'బెస్టియల్',
  'బెస్టియాలిటీ',
  'బొచ్చు',
  'బోల్లక',
  'బ్లడీ',
  'భావప్రాప్తి',
  'మనసు పడ్డట్లు చిత్రించారు',
  'మింగడం',
  'మూకదాడులు',
  'మూల',
  'మృగ',
  'యురేత్రా',
  'యూరినరీ',
  'యోని',
  'రంప్',
  'రగిలిపోతోంది',
  'రాపర్',
  'రిటార్డ్',
  'రిమ్',
  'రిమ్మింగ్',
  'రేపిస్ట్',
  'రేప్',
  'రేసీ',
  'రోసెట్',
  'లింగం',
  'లించ్',
  'లైంగిక సంతృప్తి కోసం స్త్రీలు ఉపయోగించే పురుషాంగము వంటి పరికరము',
  'వక్షోజాలను',
  'వాంగ్',
  'వికృతత్వం',
  'విసర్జన',
  'వీర్యం',
  'వృషణ',
  'వృషణం',
  'వృషణాలు',
  'వేశ్య',
  'వ్యాంక్',
  'శాడిజం',
  'శాడిస్ట్',
  'శిశ్న',
  'శృంగార',
  'శ్రేష్ఠత',
  'షగ్గింగ్',
  'షిట్టింగ్',
  'షిట్టెడ్',
  'షిట్స్',
  'షీమేల్',
  'షైట్',
  'సక్',
  'సిగ్ హీల్',
  'సెక్స్',
  'సోడోమి',
  'సోడోమైజ్',
  'సోడోమైజ్డ్',
  'సోడోమ్',
  'స్కలనం',
  'స్కాన్క్',
  'స్క్రోటమ్',
  'స్ఖలనం',
  'స్ఖలనాలు',
  'స్ఖలించు',
  'స్త్రీగుహ్యాంకురము',
  'స్నాచ్',
  'స్పంకీ',
  'స్పంక్',
  'స్పెర్మా',
  'స్పెర్మ్',
  'స్మట్',
  'స్లట్స్',
  'హస్త ప్రయోగం',
  'హస్తప్రయోగం',
  'హిట్లర్',
  'హిట్లర్ను స్తుతించు',
  'హెబిఫిల్స్',
  'హెబెఫిలిక్',
  'హెబెఫీలియా',
  'హోర్',
];
